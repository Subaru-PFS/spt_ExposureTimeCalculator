...
This script regex-parses the C source files ``src/gsetc.c`` and
``src/modeldata.h`` for the hard-coded numeric arrays that the C engine uses
as lookup tables (sky spectral atlases, atmospheric transmission grids,
silicon optical constants, and the Milky Way dust extinction law), and
serializes them to a single compressed ``.npz`` archive plus a provenance
``README.md``.

It is meant to be run exactly **once**, before the C sources are removed
from the repository (see task T15 of the pure-Python port plan), so that the
pure-Python engine can load the same numbers without re-parsing C code. The
script itself is committed to document how the archive was produced and to
allow re-extraction if the C sources are ever revisited (e.g. from an older
git revision) for validation.
...
GSETC_C = REPO_ROOT / "src" / "gsetc.c"
MODELDATA_H = REPO_ROOT / "src" / "modeldata.h"
...
def _extract_c_array(text: str, name: str, ctype: str = r"(?:double|long)") -> np.ndarray:
...
def extract_oh_data(modeldata_text: str) -> np.ndarray:
    ...
    if flat.size != N_IR_OH_LINE * 2:
        raise ValueError(
            f"OHDATA: expected {N_IR_OH_LINE * 2} values, got {flat.size}"
        )

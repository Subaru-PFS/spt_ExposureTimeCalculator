...
int main(void) {
  ...
  scanf("%lg", &(obs.seeing_fwhm_800)); 

  printf("Enter zenith angle [degrees]: ");
  scanf("%lg", &(obs.zenithangle));
...
#define REF_SIZE 0.30
  if (strcmp("-",OutFileSNR)!=0) {
    snrType = 2;
    printf("Computing [OII] ELG SNR curve for fiducial parameters ...\n");
  ...
  if (strcmp("-",OutFileSNR2)!=0) {
    snrType = 0;
    printf("Computing ELG SNR curve for f=%.3e [erg cm-2 s-1],  sigma=%.1lf [km s-1] ...\n", flux_emi, sigma_emi);
...
#undef REF_SIZE
  ...
  if (strcmp("-",OutFileSNRAB)!=0) {
    snrType = 0;
    printf("Computing continuum SNR curve for fiducial parameters ...\n");
    ...
    for(ia=0;ia<spectro.N_arms;ia++) {
      ...
      for(j=0;j<spectro.npix[ia];j++) {
        fprintf(stderr, "      --> %.0f percent done ...\r",(1+ia)*(1+j)*0.008138);
  ...
  if (strcmp("-",InFileOII)!=0) {
    snrType = 2;
    printf("Processing [OII] emitter catalog ...\n");

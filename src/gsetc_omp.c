...
double getJ0(double x)
{
  ...
  {
    u = 3. / x;
    f = 0.79788456 - 0.00000077 * u - 0.00552740 * u * u - 0.00009512 * u * u * u + u * u * u * u * (0.00137234 - 0.00072805 * u + 0.00014476 * u * u);
    t = x - 0.78539816 - 0.04166397 * u - 0.00003954 * u * u + 0.00262573 * u * u * u + u * u * u * u * (-0.00054125 - 0.00029333 * u + 0.00013558 * u * u);
...
double getJ1(double x)
{
  ...
  {
    u = 3. / x;
    f = 0.79788456 + 0.00000156 * u + 0.01659667 * u * u + 0.00017105 * u * u * u + u * u * u * u * (-0.00249511 + 0.00113653 * u - 0.00020033 * u * u);
    t = x - 2.35619449 + 0.12499612 * u + 0.0000565 * u * u - 0.00637879 * u * u * u + u * u * u * u * (0.00074348 + 0.00079824 * u - 0.00029166 * u * u);
...
double gsOP_Si_abslength(double lambda, double T)
{
  ...
  for (i = 0; i < 2; i++)
    E_g[i] = E_g0[i] - beta * T * T / (T + gamma);
  alpha = 0.;
  if (hnu > E_gd)
    alpha += A_d * sqrt(hnu - E_gd);
  for (i = 0; i < 2; i++)
    for (j = 0; j < 2; j++)
    {
      de = hnu - E_g[j] + E_p[i];
      if (de > 0)
        alpha += C[i] * A[j] * de * de / (exp(E_p[i] / k / T) - 1);
      de = hnu - E_g[j] - E_p[i];
      if (de > 0)
        alpha += C[i] * A[j] * de * de / (1 - exp(-E_p[i] / k / T));
    }
...
double gsOP_Si_indexreal(double lambda, double T)
{
  ...
  return (
      -xfrac * (xfrac - 1) * (xfrac - 2) / 6. * si_table[xint - 1] + (xfrac * xfrac - 1.) * (xfrac - 2) / 2. * si_table[xint] - xfrac * (xfrac + 1) * (xfrac - 2) / 2. * si_table[xint + 1] + xfrac * (xfrac * xfrac - 1) / 6. * si_table[xint + 2]);
}
...
double gsAtmContOp(OBS_ATTRIB *obs, double lambda, unsigned long flags)
{
  ...
  case 0x0:
    ...
    k = 0.05;
    if (lambda < 900)
      k = 0.07 - 0.02 * (lambda - 800.) / 100.;
    if (lambda < 800)
      k = 0.10 - 0.03 * (lambda - 700.) / 100.;
    if (lambda < 700)
      k = 0.11 - 0.01 * (lambda - 650.) / 50.;
    if (lambda < 650)
      k = 0.11 - 0.00 * (lambda - 600.) / 50.;
    if (lambda < 600)
      k = 0.12 - 0.01 * (lambda - 550.) / 50.;
    if (lambda < 550)
      k = 0.13 - 0.01 * (lambda - 500.) / 50.;
    if (lambda < 500)
      k = 0.17 - 0.04 * (lambda - 450.) / 50.;
    if (lambda < 450)
      k = 0.25 - 0.08 * (lambda - 400.) / 50.;
    if (lambda < 400)
      k = 0.30 - 0.05 * (lambda - 380.) / 20.;
    if (lambda < 380)
      k = 0.37 - 0.07 * (lambda - 360.) / 20.;
    if (lambda < 360)
      k = 0.51 - 0.14 * (lambda - 340.) / 20.;
    if (lambda < 340)
      k = 0.82 - 0.31 * (lambda - 320.) / 20.;
    if (lambda < 320)
      k = 1.37 - 0.55 * (lambda - 310.) / 10.;
    if (lambda < 310)
      k = 1.37;
    break;
                             ...
                             double r_eff, double decent, double fieldang, unsigned long flags)
{
  ...
  else
  {
    ...
    {
      sigma = spectro->rms_spot[i] + (spectro->rms_spot[i + 1] - spectro->rms_spot[i]) * (4 * fieldang / spectro->rfov - i);
      EFL = spectro->EFL[i] + (spectro->EFL[i + 1] - spectro->EFL[i]) * (4 * fieldang / spectro->rfov - i);
    }
  }
  ...
  for (iu = 0; iu < Nu; iu++)
  {
    ...
    theta_D = 0.;
#endif
    Gtilde = exp(-k * k * sigma * sigma / 2. - pow(u / uscale, 1.666666666666666666666666667)) * getJ0(k * decent) * exp(-4. / M_PI * u * theta_D);
...
double gsAeff(SPECTRO_ATTRIB *spectro, OBS_ATTRIB *obs, int i_arm, double lambda, double fieldang)
{
  ...
  else
  {
    ...
    {
      Vig = spectro->vignette[i] + (spectro->vignette[i + 1] - spectro->vignette[i]) * (4 * fieldang / spectro->rfov - i);
    }
  }
  Aeff *= Vig;
                ...
                double *Noise, double *SkyMod, double t_exp, unsigned long flags)
{
  ...
  case 0x1:
#pragma omp parallel for private(iline, lambda, pos, count, iref, j, FR)
    for (iline = 0; iline < gsSKY_UVES_NLINES; iline++)
    {
      ...
      if (pos > -(SP_PSF_LEN / 2) && pos < Npix + SP_PSF_LEN / 2 - 1)
      {
        ...
        count = gsSKY_UVES_INT[iline] * lambda * 1e-12 * PHOTONS_PER_ERG_1NM * gsFracTrace(spectro, obs, i_arm, lambda, 1) * gsAeff(spectro, obs, i_arm, lambda, fieldang) * t_exp * M_PI * rad * rad;
...
#pragma omp parallel for private(iline, lambda, pos, count, iref, j, FR)
    for (iline = 0; iline < N_IR_OH_LINE; iline++)
    {
      ...
      if (pos > -(SP_PSF_LEN / 2) && pos < Npix + SP_PSF_LEN / 2 - 1)
      {
        ...
        count = OHDATA[2 * iline + 1] * lambda * 1e-12 * PHOTONS_PER_ERG_1NM * gsFracTrace(spectro, obs, i_arm, lambda, 1) * gsAeff(spectro, obs, i_arm, lambda, fieldang) * t_exp * M_PI * rad * rad;
        if (count < 0)
          count = 0;
        ...
        count *= airmass * exp(-gsAtmContOp(obs, lambda, flags) * airmass / 1.086) * exp((14.8 - 15.8) / 1.086);
...
#pragma omp parallel for private(lambda, j, trans, mag, lunar_cont, lunarphase, scale_RS, scale_MS, kV, alpha, Istar, f1, f2, Bmoon, count, FR) reduction(+ : num, den) reduction(* : continuum)
  for (ipix = 0; ipix < Npix; ipix++)
  {
    ...
    case 0x1:
    case 0x2:
      continuum = lambda < 375 ? 0.17 : lambda < 483 ? 0.14
                                    : lambda < 580   ? 0.09
                                    : lambda < 674.5 ? 0.10
                                    : lambda < 858   ? 0.08
                                                     : 0.07;
      continuum /= 1.1;
      if (lambda > 1040 && ((obs->skytype & 0xf) == 0x1))
        continuum = 0.4669 * (1000. / lambda) * (2.0 * lambda / 1000. - 0.5);
    ...
    if (obs->lunarZA < 90)
    {
      ...
      case 0x0:
        ...
        Bmoon = (f1 * scale_RS + f2 * scale_MS) * Istar * pow(10., -0.4 * kV / sqrt(1. - 0.96 * sin(obs->lunarZA * DEGREE) * sin(obs->lunarZA * DEGREE))) * (1. - pow(10., -0.4 * kV / sqrt(1. - 0.96 * sin(obs->zenithangle * DEGREE) * sin(obs->zenithangle * DEGREE))));
    ...
    count = continuum * dl * gsAeff(spectro, obs, i_arm, lambda, fieldang) * t_exp * M_PI * rad * rad * gsFracTrace(spectro, obs, i_arm, lambda, 1);
  ...
  var = (spectro->dark[i_arm] * t_exp * sample_factor + spectro->read[i_arm] * spectro->read[i_arm]) * spectro->width[i_arm];
  for (ipix = 0; ipix < Npix; ipix++)
    Noise[ipix] += var;
...
#define NP_WIN 32
  ...
  counts = F * trans * pow(10., -0.4 * gsGalactic_Alambda__EBV(lambda) * obs->EBV) * gsGeometricThroughput(spectro, obs, lambda, r_eff, decent, fieldang, flags) * gsFracTrace(spectro, obs, i_arm, lambda, 0) * PHOTONS_PER_ERG_1NM * lambda * t_exp * gsAeff(spectro, obs, i_arm, lambda, fieldang) * 1e4;
                ...
                double t_exp, unsigned long flags, int snrType)
{
  ...
  if (snrType == 0)
  {
    SNR = 0;
    for (ipix = 0; ipix < Npix; ipix++)
      SNR += Signal[ipix] * Signal[ipix] / Noise[ipix];
    SNR = sqrt(SNR);
  }
                       ...
                       double t_exp, unsigned long flags, int snrType)
{
  ...
  counts = src_cont * trans * pow(10., -0.4 * gsGalactic_Alambda__EBV(lambda) * obs->EBV) * gsGeometricThroughput(spectro, obs, lambda, r_eff, decent, fieldang, flags) * gsFracTrace(spectro, obs, i_arm, lambda, 0) * PHOTONS_PER_ERG_1NM * lambda * t_exp * gsAeff(spectro, obs, i_arm, lambda, fieldang) * 1e4;
#ifdef HGCDTE_SUTR
  if (spectro->Dtype[i_arm] == 1)
    counts *= 1.2;
#endif
                    ...
                    double fieldang, double *Noise, double t_exp, unsigned long flags, int snrType)
{
  ...
  counts = src_cont * trans * pow(10., -0.4 * gsGalactic_Alambda__EBV(ll) * obs->EBV) * gsGeometricThroughput(spectro, obs, ll, r_eff, decent, fieldang, flags) * gsFracTrace(spectro, obs, i_arm, ll, 0) * PHOTONS_PER_ERG_1NM * ll * t_exp * gsAeff(spectro, obs, i_arm, ll, fieldang) * 1e4;
#ifdef HGCDTE_SUTR
  if (spectro->Dtype[i_arm] == 1)
    counts *= 1.2;
#endif
  ...
  if (snrType == 2)
  {
    ...
    SNR = 0;
    for (ipix = 0; ipix < Npix; ipix++)
      SNR += (Signal0[ipix] + Signal1[ipix]) * (Signal0[ipix] + Signal1[ipix]) / myNoise[ipix];
    SNR = sqrt(SNR);
    free((char *)Signal0);
    free((char *)Signal1);
  }
...
void gsReadSpectrographConfig(char FileName[], SPECTRO_ATTRIB *spectro, double degrade)
{
  ...
  spectro->D_outer = -1;
  for (i = 0; i < 5; i++)
    spectro->rms_spot[i] = -1;
  spectro->fiber_ent_rad = -1;
  spectro->N_arms = 0;
  for (i = 0; i < 5; i++)
    spectro->vignette[i] = 1.;
  for (i = 0; i < MAXARM; i++)
    spectro->Dtype[i] = 0;
...
int main(void)
{
  ...
  int flag_reused;
  double lambda, t, fieldang, decent;
  double z;
  ...
  for (ia = 0; ia < spectro.N_arms; ia++)
    spNoise[ia] = (double *)malloc((size_t)(spectro.npix[ia] * sizeof(double)));
  spSky = (double **)malloc((size_t)(spectro.N_arms * sizeof(double *)));
  for (ia = 0; ia < spectro.N_arms; ia++)
    spSky[ia] = (double *)malloc((size_t)(spectro.npix[ia] * sizeof(double)));
...
#define REF_SIZE 0.30
  if (strcmp("-", OutFileSNR) != 0)
  {
    ...
    double zmax = 2.5001;
    double dz = 0.0002;
    int nz = (int)round((zmax - zmin) / dz);
    double zarr[nz];
    double Aeff_arr[nz];
    double snr_arm_arr[nz][spectro.N_arms];
    double snr_tot_arr[nz];
    double fiber_aperture_arr[nz];
...
#pragma omp parallel for ordered private(z, ia) reduction(+ : snrtot, Aeff)
    for (int iz = 0; iz < nz; iz++)
    {
      ...
      for (ia = 0; ia < spectro.N_arms; ia++)
      {
        ...
        {
          snr[ia] = gsGetSNR_OII(&spectro, &obs, ia, z, flux_emi, sigma_emi, ref_input, 0., 1., decent, fieldang, spNoise[ia], t, 0x0, snrType) * sqrt((double)n_exp);
          snr_arm_arr[iz][ia] = snr[ia];
        }
      ...
      Aeff_arr[iz] = Aeff;
      snr_tot_arr[iz] = snrtot;
      fiber_aperture_arr[iz] = gsGeometricThroughput(&spectro, &obs, 372.845 * (1 + z), ref_input, decent, 0, 0x0);
    }
    ...
    for (int iz = 0; iz < nz; iz++)
    {
      fprintf(fp, "%6.4lf %7.2lf %7.2lf %8.6lf %8.5lf",
              zarr[iz], 372.71 * (1 + zarr[iz]), 372.98 * (1 + zarr[iz]), fiber_aperture_arr[iz], Aeff_arr[iz]);
      for (ia = 0; ia < spectro.N_arms; ia++)
      {
        fprintf(fp, " %8.4lf", snr_arm_arr[iz][ia]);
      }
      fprintf(fp, " %8.4lf\n", snr_tot_arr[iz]);
    }
#ifndef QUIET_FOR_OMP
  ...
  if (strcmp("-", OutFileSNR2) != 0)
  {
    ...
    double snr_tot_arr[nz];
    double fiber_aperture_arr[nz];
    const double lambda_line_min = 345.5;
...
#pragma omp parallel for ordered private(z, ia) reduction(+ : snrtot, Aeff)
    for (int iz = 0; iz < nz; iz++)
    {
      ...
      {
        snr[ia] = 0.;
        if (spectro.lmin[ia] < lambda_line_min * (1 + z) && lambda_line_min * (1 + z) < spectro.lmax[ia])
        {
          snr[ia] = gsGetSNR_Single(&spectro, &obs, ia, mag, lambda_line_min * (1 + z), flux_emi, sigma_emi, ref_input, decent, fieldang, spNoise[ia], t, 0x0, 0) * sqrt((double)n_exp);
          Aeff += gsAeff(&spectro, &obs, ia, lambda_line_min * (1 + z), fieldang);
        }
        snrtot += snr[ia] * snr[ia];
        snr_arm_arr[iz][ia] = snr[ia];
      }
      ...
      Aeff_arr[iz] = Aeff;
      snr_tot_arr[iz] = snrtot;
      fiber_aperture_arr[iz] = gsGeometricThroughput(&spectro, &obs, lambda_line_min * (1 + z), ref_input, decent, fieldang, 0x0);
    }

    for (int iz = 0; iz < nz; iz++)
    {
      fprintf(fp, "%7.2lf %8.6lf %8.5lf", lambda_line_min * (1 + zarr[iz]), fiber_aperture_arr[iz], Aeff_arr[iz]);
      for (ia = 0; ia < spectro.N_arms; ia++)
      {
        fprintf(fp, " %8.4lf", snr_arm_arr[iz][ia]);
      }
  ...
  if (strcmp("-", InFileOII) != 0)
  {
    ...
    for (z = 0.1; z < 2.5001; z += 0.00025)
    {
      ...
      for (ia = 0; ia < spectro.N_arms; ia++)
      {
        snr[ia] = 0.;
        if (spectro.lmin[ia] < 373.8 * (1 + z) && 371.8 * (1 + z) < spectro.lmax[ia])
          snr[ia] = gsGetSNR_OII(&spectro, &obs, ia, z, 1e-16, 70., 0.0, 0.0, 1.0, decent, fieldang, spNoise[ia], t, 0x0, snrType) * sqrt((double)n_exp) / 1.6;
        snrtot += snr[ia] * snr[ia];
      }
      snrtot = sqrt(snrtot);
      if (snrtot > snrmax16)
                  ...
                  &id, &z, &r_eff, &ROII, &FOII, &contOII, &sigma) != EOF)
    {
      ...
      snrtot = 0.;
      if (FOII >= mdlf && r_eff >= 0.)
        for (ia = 0; ia < spectro.N_arms; ia++)
        {
          snr[ia] = 0.;
          if (spectro.lmin[ia] < 373.8 * (1 + z) && 371.8 * (1 + z) < spectro.lmax[ia])
            snr[ia] = gsGetSNR_OII(&spectro, &obs, ia, z, FOII, 70., r_eff, contOII, ROII, decent, fieldang, spNoise[ia], t, 0x0, snrType) * sqrt((double)n_exp);
          snrtot += snr[ia] * snr[ia];
        }
      snrtot = sqrt(snrtot);

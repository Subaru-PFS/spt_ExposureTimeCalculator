...
"""Deprecated argparse/`@file`-style entry point for `pfsspecsim.sim.pfsspec`
...
``.defaults`` file). New code should prefer the ``pfs-spec sim`` CLI
(`pfsspecsim.cli.sim`); see the README's old -> new parameter migration
table.
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="PFS Spectral Simulator developed by Kiyoto Yabe and Robert Lupton",
    ...
    parser.add_argument(
        "--outDir", type=str, help="Directory for outputs", default="out"
    )
    ...
    parser.add_argument(
        "--objId", type=int, help="object id of first realisation ", default=1
    )
    parser.add_argument("--catId", type=int, help="catalogue id", default=0)
    parser.add_argument(
        "--spectrograph", type=int, help="spectrograph number", default=1
    )
    parser.add_argument(
        "--countsMin",
        type=float,
        help="Minimum counts per pixel for noise",
        default=0.1,
    )
    parser.add_argument(
        "--asciiTable",
        type=str,
        help="simulated spectrum output ASCII file",
        default="None",
    )
    parser.add_argument(
        "--writeFits", type=str, help="Write FITS files", default="True"
    )
    parser.add_argument(
        "--writePfsArm",
        type=str,
        help="Write pfsArm files (writeFits must be set)",
        default="True",
    )
    parser.add_argument(
        "--plotArmSet", action="store_true", help="Plot the pfsArmSet data"
    )
    parser.add_argument(
        "--plotObject", action="store_true", help="Plot the pfsObject data"
    )
